...
HOST_ENVVARS[TOKEN_AUTH_REALM]=api # This is slightly different, needs to be http://api.<uuid>.<tld>/auth/v1/token
HOST_ENVVARS[TOKEN_AUTH_ISSUER]=api
...
if [[ -n "$BALENA_TLD" ]]; then
  for VARNAME in "${!HOST_ENVVARS[@]}"; do
      ...
      if [[ -z "$VARVALUE" ]]; then
          ...
          if [ "$VARNAME" == "BALENA_TOKEN_AUTH_REALM" ] || [ "$VARNAME" == "TOKEN_AUTH_REALM" ]; then
